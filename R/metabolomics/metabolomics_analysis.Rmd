...
metabolomics_cleaned = c18positive_gathered_combined_cleaned_impute$data %>%
  as.data.frame() %>%
  select(c(metabolomics_top_variance_cols))
...
# mbx_meta_df %>% mutate(final_grp_word = str_replace_all(final_grp, ' ', '_')) %>% pull(final_grp_word)

int_list = interaction(mbx_meta_df$responder_status_delta, mbx_meta_df$phase)
tg = mbx_meta_df %>% mutate(final_grp_word = str_replace_all(final_grp, ' ', '_')) %>% pull(final_grp_word)
tg = factor(tg, levels=c('Placebo', 'Low_Dose_2FL', 'High_Dose_2FL'))
rs = mbx_meta_df$responder_status_delta
ph = mbx_meta_df$phase

mm = model.matrix(~tg*ph)
pheatmap(mm)

mbx_abund_matrix_filtered[is.na(mbx_abund_matrix_filtered)] = 0
mbx_abund_matrix_filtered_renamed = mbx_abund_matrix_filtered %>%
  rownames_to_column(var='sample_id') %>%
  ungroup() %>% 
...
voom.y.d = voom(mbx_abund_matrix_filtered_renamed, mm, plot=T)

fit = lmFit(voom.y.d, mm, block=participant_id)
coef.fit = fit$coefficients
head(coef(fit))

fit <- eBayes(fit)
anova.res <- topTable(fit, n=Inf) %>%
...
# 
# 
# contr = makeContrasts(int_listresponder.Intervention-int_listnonresponder.Intervention, levels=colnames(coef(fit)))
# 
# tmp = contrasts.fit(fit, contr)
# tmp = eBayes(tmp)
# top_table = topTable(tmp, sort.by='P', n=Inf)
# 
# coef_deg = coef.fit[rownames(coef.fit) %in% rownames(top_table)[1:4],]


```

```{r }
design <- model.matrix(~0+mbx_meta_df$participant_id)
design <- cbind(design, X= mbx_meta_df %>% mutate(X = ifelse(final_grp == 'Placebo' & phase == 'Intervention', 1, 0)) %>% pull(X))
design <- cbind(design, Y= mbx_meta_df %>% mutate(X = ifelse(final_grp == 'Low Dose 2FL' & phase == 'Intervention', 1, 0)) %>% pull(X))
design <- cbind(design, Z= mbx_meta_df %>% mutate(X = ifelse(final_grp == 'High Dose 2FL' & phase == 'Intervention', 1, 0)) %>% pull(X))

dim(design)

voom.y.d = voom(mbx_abund_matrix_filtered_renamed, design, plot=T)

fit = lmFit(voom.y.d, design)
coef.fit = fit$coefficients
head(coef(fit))
...
sam_mat = metabolomics_cleaned %>%
  rownames_to_column(var='sample_id') %>%
  gather(key='metabolite', value='peak_height', -sample_id) %>%
  rowwise() %>% 
  mutate(peak_height = log10(peak_height + 10)) %>% 
  spread(key='metabolite', value='peak_height') %>%
  filter(grepl('_2', sample_id) | grepl('_4', sample_id)) %>%
  rowwise() %>%
  mutate(participant_id = strsplit(sample_id, '_')[[1]][1]) %>%
  filter(participant_id %in% (bifido_responders %>% filter(responder_status_delta == 'responder') %>% pull(participant_id)))  %>%
  select(-participant_id) %>%
  column_to_rownames(var='sample_id') %>% 
  t() 

sam_labels = colnames(sam_mat) %>% as.data.frame() %>% rename(sample_id = 1) %>% rowwise() %>% mutate(tp = as.numeric(strsplit(sample_id, '_')[[1]][2]), tp = tp/2) %>% pull(tp)

sam.out = sam(sam_mat, sam_labels,rand=123)
...
samsum = summary(sam.out, 0.7)

samsum@row.sig.genes
...
placebo_v_highdose_mas_metabolomics_grp$results %>%
  filter(value == 'phase_grp') %>%
  mutate(qval = p.adjust(pval, method='BH')) %>%
  arrange(qval, pval)
...
placebo_v_lowdose_mas_metabolomics_grp$results %>%
  filter(value == 'phase_grp') %>%
  mutate(qval = p.adjust(pval, method='BH')) %>%
  arrange(qval, pval)
...
highdose_v_lowdose_mas_metabolomics_grp$results %>%
  filter(value == 'phase_grp') %>%
  mutate(qval = p.adjust(pval, method='BH')) %>%
  arrange(qval, pval)
