...
top_var_cytos = olink_results_cleaned %>%
  summarise_if(is.numeric, var) %>%
  gather(key='cyto', value='var') %>%
  arrange(desc(var)) %>%
  # top_frac(0.75) %>%
  pull(cyto)
...
placebo_v_highdose_mas_treatment_grp$results %>%
  filter(value == 'phase_grp') %>%
  mutate(qval = p.adjust(pval, method='BH')) %>%
  arrange(qval, pval)
...
placebo_v_lowdose_mas_treatment_grp$results %>%
  filter(value == 'phase_grp') %>%
  mutate(qval = p.adjust(pval, method='BH')) %>%
  arrange(qval, pval)
...
highdose_v_lowdose_mas_treatment_grp$results %>%
  filter(value == 'phase_grp') %>%
  mutate(qval = p.adjust(pval, method='BH')) %>%
  arrange(qval, pval)
