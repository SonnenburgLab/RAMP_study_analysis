...
main_df <- sample_df %>% 
  ...
  tidyr::separate(col = temp1,
                  into = c("Timepoint", "Part", "TreatmentArm", "ParticipantTimepoint"), 
                  sep='-', remove = FALSE) %>%
  dplyr::select(c(-temp1, -temp2, -temp3, -Sample))

meta_df = main_df %>%
  dplyr::select(ParticipantTimepoint, TreatmentArm, Timepoint) %>%
  distinct()
```

```{r separate_basal}
basal_main_df <- main_df %>% dplyr::filter(Condition == "US") %>% arrange(ParticipantTimepoint)
basal_main_numeric <- basal_main_df %>% dplyr::select(-(Timepoint:Condition), -c(Plate, Control))
```
...
for (condition in conditions){
  cond_df <- main_df %>% 
    ...
    dplyr::select(-(Timepoint:Condition), -c(Plate, Control))
  metadata <- main_df %>% 
    dplyr::filter(Condition == condition) %>% 
    arrange(ParticipantTimepoint) %>%
    dplyr::select(Timepoint:Condition)
...
phosphoflow_data.imputed = fold_main_conditions_df %>%
  ...
  dplyr::select(participant_timepoint, everything(), -c(biobank_id, ParticipantTimepoint, Participant, timepoint, participant_id)) %>%
...
phosphoflow_data_cleaned_scaled = phosphoflow_data.imputed$data %>%
  as.data.frame() %>%
  dplyr::select(c(phosphoflow_data_top_variance_cols)) %>%
  scale() %>%
  as.data.frame()
...
mas_phosphoflow_trt_grp <- Maaslin2(
...
mas_phosphoflow_trt_grp$results %>% 
  arrange(pval, qval)

mas_phosphoflow_responder_grp <- Maaslin2(
...
mas_phosphoflow_responder_grp$results %>% 
  arrange(pval, qval)
...
CRS_feature_df <- dplyr::select(fold_main_conditions_df, c(any_of(CRS_features)))
...
CRS_df <- data.frame(dplyr::select(fold_main_conditions_df, ParticipantTimepoint), CRS_scores) %>%
  left_join(meta_df) %>%
  rowwise() %>%
  mutate(Participant = str_split(ParticipantTimepoint, '[[:letter:]]')[[1]][1]) %>%
  dplyr::select(Participant, Timepoint, TreatmentArm, CRS_scores) %>%
...
CRS_df_all = data.frame(dplyr::select(fold_main_conditions_df, ParticipantTimepoint), CRS_scores) %>%
  left_join(meta_df) %>%
  rowwise() %>%
  mutate(Participant = str_split(ParticipantTimepoint, '[[:letter:]]')[[1]][1]) %>%
  dplyr::select(Participant, Timepoint, TreatmentArm, CRS_scores) %>%
...
CRS_df_responder = CRS_df_all %>% 
  mutate(
    biobank_id = as.character(Participant),
    biobank_id = ifelse(nchar(biobank_id) == 2, paste0('0', biobank_id), biobank_id),
    biobank_id = ifelse(nchar(biobank_id) == 1, paste0('00', biobank_id), biobank_id),
    biobank_id = paste0('47252-0', biobank_id)) %>%
  left_join(ramp_metadata_df, by='biobank_id') %>% 
  filter(!is.na(responder_status_delta))

ggplot(CRS_df_responder, aes(x=Timepoint, y=CRS_scores)) +
  geom_point(aes(group=Participant),size=2, alpha=0.7) +
  geom_line(aes(group=Participant), alpha=0.7) +
  geom_boxplot(aes(fill=responder_status_delta), outlier.size=0) +
...
wilcox.test(CRS_df_responder %>% filter(responder_status_delta == 'responder' & Timepoint == 'Baseline') %>% pull(CRS_scores),
            CRS_df_responder %>% filter(responder_status_delta == 'responder' & Timepoint == 'End') %>% pull(CRS_scores),
            paired=T)

wilcox.test(CRS_df_responder %>% filter(responder_status_delta == 'nonresponder' & Timepoint == 'Baseline') %>% pull(CRS_scores),
            CRS_df_responder %>% filter(responder_status_delta == 'nonresponder' & Timepoint == 'End') %>% pull(CRS_scores),
            paired=T)

wilcox.test(CRS_df_responder %>% filter(responder_status_delta == 'responder' & Timepoint == 'End') %>% pull(CRS_scores),
            CRS_df_responder %>% filter(responder_status_delta == 'nonresponder' & Timepoint == 'End') %>% pull(CRS_scores))
